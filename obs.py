...
class Obs:
    ...
    def __init__(self, url, port, password, config, queue):
        self.__ws = obsws(url, port, password)
        self.currently_streaming = []
        self.__queue = queue

        self.__source_prefix = config.get('source_prefix')

        self.__ws.register(self.handle_switch_scenes, events.SwitchScenes)
        self.__ws.connect()

        # Now that we have connected, get currently streaming computers
        self.get_currently_streaming_computers()

    def handle_switch_scenes(self, message):
        def handler():
            self.get_currently_streaming_computers()
        ...
        def handle_scene(scene):
            for source in scene['sources']:
                ...
                if type == 'ffmpeg_source':
                    name = source['name']
                    if name.startswith(self.__source_prefix):
                        index = int(name[len(self.__source_prefix):])
